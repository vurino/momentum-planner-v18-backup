...
import {
  ...
  RefreshControl, Pressable, Dimensions, Modal,
...
import {
  useTheme, getTaskGlow, getNeuShadow, SPACING, RADIUS, FONT,
      ...
      {weekDays.map((day, i) => {
        ...
        return (
          <TouchableOpacity key={i} onPress={() => onDayPress(day)} style={styles.dayCol} activeOpacity={0.7}>
            <Text style={[styles.dayLabel, { color: colors.dayLabelColor }]}>{days[i]}</Text>
            <View style={[
              styles.dayBubble,
              isToday && { backgroundColor: colors.accent },
              !isToday && isDone && { backgroundColor: colors.dayBubbleDone, borderWidth: 1, borderColor: colors.done + '40' },
              !isToday && !isDone && {
  ...
  return (
    <View style={styles.ringWrap}>
      <View style={styles.ringSvgWrap}>
        <View style={[styles.ringOuter, { width: size, height: size, borderRadius: size / 2, borderColor: colors.progressTrack, borderWidth: 3 }]}>
      ...
      <View>
        <Text style={[styles.ringMain, { color: colors.textBody }]}>
          <Text style={{ color: colors.textPrimary, fontWeight: '600' }}>{completed} of {total}</Text>{' done'}
        </Text>
        <Text style={[styles.ringSub, { color: colors.textDim }]}>{total - completed} remaining today</Text>
      </View>
    </View>
  );
};
...
const TaskItem = ({ task, onToggle, onFocus, onNotes, colors, isDark }: {
  ...
  return (
    <View style={[
      styles.taskRow,
      isCur && !isDone && [styles.taskRowCurrent, { backgroundColor: colors.bgTask }, getTaskGlow()],
      ...
      <View style={styles.taskActions}>
        {isCur && !isDone && (
          <TouchableOpacity onPress={() => onFocus(task)} style={[styles.focusBadge, { backgroundColor: colors.accent }]}>
            <Text style={styles.focusBadgeText}>FOCUS</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity onPress={() => onNotes(task)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          <Ionicons
    ...
    <Modal visible={visible} transparent animationType="fade">
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
        <Pressable style={styles.modalOverlay} onPress={onClose}>
          <Pressable onPress={e => e.stopPropagation()} style={[styles.notesCard, { backgroundColor: colors.bgSurface || colors.bgBase }]}>
    ...
    <View>
      <Animated.View style={[styles.bSection, bHeaderStyle]}>
        <View style={styles.bGreeting}>
          ...
          <View style={styles.bDateRow}>
            <Text style={[styles.bDayName, { color: colors.textPrimary }]}>{dayName}</Text>
            <Text style={[styles.bDateSub, { color: colors.textDim }]}>{format(currentDate, 'MMM d')}</Text>
          </View>
        </View>
        ...
        {heroTask ? (
          <Animated.View style={heroStyle}>
            <View style={[styles.heroCard, { backgroundColor: colors.bgSurface }, getNeuShadow(isDark)]}>
              ...
              <Text style={[styles.heroTaskName, { color: colors.textPrimary }]}>{heroTask.slot.label}</Text>
              ...
              <TouchableOpacity style={[styles.heroFocusBtn, { backgroundColor: colors.accent }]} onPress={() => handleFocus(heroTask)}>
            ...
            {upNext.map((t, i) => (
              <View key={t.id}>
                <View style={styles.upNextRow}>
                  <View style={[styles.upNextDot, { backgroundColor: i === 0 ? colors.accent : colors.textInvisible }]} />
                  <Text style={[styles.upNextName, { color: i === 0 ? colors.textBody : colors.textDim }]} numberOfLines={1}>
                    {t.slot.label}
                  </Text>
                  <Text style={[styles.upNextTime, { color: colors.textInvisible }]}>{t.slot.start_time}</Text>
                </View>
                {i < upNext.length - 1 && <View style={[styles.upNextDivider, { backgroundColor: colors.divider }]} />}
              </View>
            ))}
          </View>
        )}
      ...
      <Animated.View style={[styles.aHeader, aHeaderStyle]} pointerEvents="none">
        <View style={styles.aDateBlock}>
          <Text style={[styles.aDayName, { color: colors.textPrimary }]}>{dayName}</Text>
          <Text style={[styles.aDateSub, { color: colors.textMuted }]}>{format(currentDate, 'MMMM d')} · Momentum Planner</Text>
        </View>
        <WeekStrip currentDate={currentDate} onDayPress={setCurrentDate} completedDates={completedDates} colors={colors} isDark={isDark} />
        <View style={styles.aProgressRow}>
          <View style={[styles.aProgressTrack, { backgroundColor: colors.progressTrack }]}>
            <View style={[styles.aProgressFill, { width: `${progress.percentage}%` as any, backgroundColor: colors.done }]} />
          </View>
...
const styles = StyleSheet.create({
  ...
  heroCard: { borderRadius: RADIUS.xl, padding: SPACING.md, marginBottom: SPACING.md },
  heroEmpty: { borderRadius: RADIUS.xl, padding: SPACING.md, borderWidth: 1, marginBottom: SPACING.md, alignItems: 'center', justifyContent: 'center', height: 80 },
  ...
  heroFocusBtn: { alignSelf: 'flex-start', paddingHorizontal: SPACING.md, paddingVertical: 6, borderRadius: RADIUS.md },
  ...
  upNextSection: { position: 'relative', marginBottom: SPACING.md },
  ...
  aHeader: { position: 'absolute', top: 0, left: 0, right: 0, paddingTop: 8 },
  ...
  dayBubble: { width: 28, height: 28, borderRadius: 9, justifyContent: 'center', alignItems: 'center' },
  dayNum: { fontSize: 10, fontWeight: '700' },
  taskRow: { flexDirection: 'row', alignItems: 'center', gap: SPACING.sm, paddingVertical: 7, paddingHorizontal: SPACING.sm, borderRadius: RADIUS.lg, marginBottom: 4 },
  taskRowCurrent: { borderRadius: RADIUS.lg },
  taskRowDone: { opacity: 0.42 },
  checkbox: { width: 16, height: 16, borderRadius: 8, borderWidth: 1.5, justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
